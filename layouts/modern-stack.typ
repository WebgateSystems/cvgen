// Single-column modern stack. Optional sections only; theme from JSON.
...
#let timeline-entry(entry) = {
  grid(
    ...
    align(center)[
      #circle(radius: 2.4pt, fill: rgb(colors.accent))
    ],
    {
...
#if present(cv.essential_skills) or present(cv.additional_skills) {
  section-title("Key Skills")
  skill-pills(cv.essential_skills + cv.additional_skills)
}
